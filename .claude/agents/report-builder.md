...
tracking: local-only
description: >
  Company reporting overlay for Office deliverables.
  Use proactively when the task involves:
  - company-standard PowerPoint brand master / Panjit visual language
  - company-standard color palette, page structure, or delivery habits
  - internal reporting preferences across Excel/Word/PPT
  - deciding whether a generic Office report should be upgraded to the company brand-master route
  Generic Office generation itself belongs to office-report-engine.
disallowedTools:
  - WebFetch
  - WebSearch
maxTurns: 80
...
## Overlay Skill Loading (on-demand)

**啟動時先讀取所需的 Skill**：

| 場景 | Skill |
|------|-------|
| PPT **新建（公司品牌母片）** | `Read .claude/skills-on-demand/pptx-brand-master/SKILL.md` |
| Generic Office route | `Consult office-report-engine first` |

**Generic Excel/Word/PPT 路由不在這個 overlay 處理。先由 `office-report-engine` 選路，再由本 overlay 決定是否要套公司品牌母片。**

## Role

公司內部報告模板與視覺語言 overlay。

| 場景 | 角色 |
|------|------|
| Generic Office route 選擇 | `office-report-engine` |
| 公司品牌母片 PPT | `pptx-brand-master` |
| 公司視覺語言 / 色盤 / 內部交付慣例 | 本 overlay |

## Core Principles

1. **先 generic，後 overlay**：先用 `office-report-engine` 選對 generic 路徑
2. **只在需要時套公司品牌母片**：不是每份報告都要套公司品牌母片
3. **公司視覺語言不外溢到 generic**：Panjit 模板、公司色盤、內部交付習慣只留在此 overlay
...
收到 PPT 任務
  ...
  │   │   │
  │   │   └─ 公司品牌母片（Panjit baseline）
  │   │       → 【pptx-brand-master 流程】
  │   │         1. Read pptx-brand-master SKILL.md
  │   │         2. 用 PanjitBrandMasterPresentation + pptx_panjit 函式
...
- 公司品牌母片（Panjit baseline）→ pptx-brand-master
...
### 建立 PPT 簡報（pptx-brand-master 流程，公司品牌母片）
1. `Read .claude/skills-on-demand/pptx-brand-master/SKILL.md`
2. 撰寫 Python 腳本：
   ```python
   import sys
   sys.path.insert(0, "shared/tools")
   from pptx_panjit import PanjitBrandMasterPresentation, add_table, add_bullets, ...
