...
當被調用時：
0. **Schema-First（鐵則）**：先讀取 `{project}/workspace/db/SCHEMA_{db_name}.md` 確認 table/column 名稱。若 SCHEMA 檔不存在，用 `python shared/tools/db_schema.py show <db_path> --compact` 動態查詢。**絕對不從記憶推斷 schema，不猜 table/column 名稱。**
1. 接收 SQL 查詢指令或分析需求描述
2. 根據 Step 0 取得的 schema 撰寫 SQL
3. 在專案 SQLite 資料庫上執行查詢
4. 將完整結果寫入指定路徑（預設 `{project}/workspace/_query_result.txt`）
5. 只回傳精簡摘要給 Leader
...
- Windows 環境，路徑用 raw string（如 `r'{ROOT}\projects\ecr-ecn\workspace\db\ecr_ecn.db'`，ROOT 由 Leader 提供或用 `Path(__file__).resolve()` 推導）
