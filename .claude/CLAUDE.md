...
| **ingest-archiver** | data_ingestion `archive_original` 節點 — 把來源檔搬進 `{P}/vault/originals/` 並計算 SHA-256 | `.claude/agents/ingest-archiver.md` |
| **ingest-structure-detector** | data_ingestion `detect_structure` 節點 — 偵測檔案 sheet/column/type/encoding/merged-cells | `.claude/agents/ingest-structure-detector.md` |
| **ingest-exclusion-applier** | data_ingestion `apply_exclusions` 節點 — 套用 RD-/PE-/EOL/MBU3 等排除規則、處理 -AU 後綴、產生 `operation_id` | `.claude/agents/ingest-exclusion-applier.md` |
| **ingest-db-writer** | data_ingestion `ingest_to_db` 節點 — 批次 INSERT 到 SQLite 並附加 `_operation_id/_source_file/_source_version/_source_row` 追溯欄位 | `.claude/agents/ingest-db-writer.md` |
| **ingest-validator** | data_ingestion `post_validation` 節點 — 跑 checklist（NULL 率/重複/值域/追溯欄位）並產出 `checklist_responses` | `.claude/agents/ingest-validator.md` |
...
- **data_ingestion workflow 啟動後，每個節點委派對應的 node-agent 執行**（不由 Leader 直接做，規則內嵌在 agent context 避免遺忘）。節點 → agent 對應：
  - `archive_original` → `ingest-archiver`
  - `detect_structure` → `ingest-structure-detector`
  - `confirm_with_user` → Leader 自己跟使用者對齊（需要互動，不委派）
  - `apply_exclusions` → `ingest-exclusion-applier`
  - `ingest_to_db` → `ingest-db-writer`
  - `generate_schema_cache` → Leader 直接跑 `db_schema.py generate`（單一指令，無需 agent）
  - `post_validation` → `ingest-validator`
...
**啟用狀態**：`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 已在 global settings 開啟。

**何時用 Agent Teams 而非 Sub-agent：**

| 情境 | 選擇 |
|------|------|
| Teammate 之間需要直接討論、相互質疑發現 | **Agent Teams** |
| 並行調查多個競爭假設（如除錯） | **Agent Teams** |
| 只需要把結果回傳給 Leader、互不溝通 | Sub-agent |
| 單純隔離大量輸出或工具複雜度 | Sub-agent |

**ECR-ECN 適用情境：**
- 一個 ecr-explorer 跑 DB 查詢找出影響料號，同時一個 ecr-analyst 分析規範衝突，兩者可以直接互傳訊息交叉驗證 → **用 Agent Teams**
- 單純要 query-runner 跑一批 SQL 回傳摘要 → **用 Sub-agent**

**啟動方式（自然語言）：**
```
Create an agent team with an explorer and an analyst teammate.
Explorer searches for affected part numbers in the DB.
Analyst checks Q006/Q101 spec compliance.
Have them share findings directly.
```

**Windows 限制**：分割窗格模式不可用，用 in-process 模式（Shift+Down 切換 teammate）。

**成本警告**：每個 teammate 是獨立 Claude 實例，token 成本隨 teammate 數線性增加。3–5 個 teammate 是實用上限。
...
每個 project_state.md 首行標注類型：
- `<!-- type: knowledge -->` — 知識型（資料入庫、查詢、問答）；行數上限 **150 行**
- `<!-- type: project_management -->` — 專案管理型（跨領域分析、決策追蹤、進度更新）；行數上限 **200 行**，另有 backlog.db 追蹤任務

### Block Memory 架構（冷熱分離）

**熱資料**（project_state.md，每次會話讀入）：只保留當前狀態快照，嚴格區塊化：
- `## 當前階段`（≤10 行）、`## 活躍任務`（≤15 行）、`## 資料庫現況`（≤10 行）
- `## 活躍知識依賴`（≤5 行）、`## 未解問題`（≤5 條）、`## 主要產出（當前版本）`（≤10 行）

**冷資料**（按需載入）：
- `project_history.md` — 已完成任務、歷史版本說明
- `kb_index.db` — 決策/知識，用 `kb.py search` 按需查詢
- `workspace/db/backlog.db` — project_management 型的任務清單（`backlog.py list`）

**黃金規則**：已完成的事項 → `project_history.md`；決策 → `kb.py add-decision`；不要在 project_state.md 中保留歷史版本說明。
...
4. 按節點逐步完成（read_project_state → load_project_backlog → check_knowledge_health → diff_db → flag_questions → report_ready）
   - **load_project_backlog**（project_management 型自動觸發）：若有 backlog.db，顯示開放 P0/P1、blocked、stale 任務摘要
...
5. **工作區衛生檢查**（自動，非必填）→ `check_hygiene` 節點
   - validator: 掃描 workspace/_*.txt、tmp_*.txt — >5 個 WARN，>15 個 FAIL

6. **同步 Backlog**（project_management 型必填）→
   `python shared/workflows/coordinator.py complete sync_backlog`
   - 用 `backlog.py resolve A-NNN` 標記已完成任務
   - 用 `backlog.py add` 記錄新發現的待辦事項

7. **（選填）check_promote** — coordinator 自動計數，每 3 次提醒 `/promote`
...
**⚠️ 啟動入庫流程前，必須先判斷是否真的需要入庫（禁止預設走 data_ingestion）：**

| 條件 | 做法 |
|------|------|
| 小量（≤500 行）且格式複雜或人工維護的活文件 | **直接讀取、在 context 中處理、直接寫回 Excel — 不入庫、不寫腳本** |
| 需要 AI 閱讀理解後結構化（複雜格式、小量） | AI 直接讀 → 產出 JSON/結構化資料 → 視需要入庫，**禁止先寫解析腳本** |
| 大量（>500 行）且需跨表 JOIN / 多腳本共用 / 重複查詢 | 啟動 data_ingestion workflow（腳本解析路徑） |
| 大量且格式複雜（腳本難解析） | 啟動 data_ingestion workflow，**但解析方式：AI 先讀樣本 → 理解語意 → 再生成針對性腳本** |

**鐵則：不得在「是否需要入庫」尚未確認前就開始寫解析腳本或啟動 data_ingestion workflow。**
...
### Backlog 常用指令（project_management 型專案）

```bash
python shared/tools/backlog.py --project <name> summary          # 一行摘要（開放/blocked/stale 數）
python shared/tools/backlog.py --project <name> list             # 列出開放 + in_progress + blocked
python shared/tools/backlog.py --project <name> add --title "..." --priority P0
python shared/tools/backlog.py --project <name> update A-001 --status in_progress
python shared/tools/backlog.py --project <name> resolve A-001 --resolution "..."
python shared/tools/backlog.py --project <name> remind           # 列出 P0/blocked>3d/stale>7d
python shared/tools/backlog.py --project <name> discuss --topic "..." --summary "..."
```
