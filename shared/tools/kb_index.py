...
Source of truth: .md files in shared/kb/
This tool builds a derived index for efficient querying, tracing, and validation.
...
ROOT = SCRIPT_DIR.parent.parent  # D:\AI_test
...
class KBIndex:
    ...
    def _parse_learning(self, path):
        ...
        for block in blocks:
            ...
            status_match = re.search(r'<!--\s*status:\s*(active|promoted|obsolete)\s*-->', block)
            if status_match:
    ...
    def generate_index(self, output_path=None):
        ...
        if skills_dir.is_dir():
            for idx, sd in enumerate(sorted(skills_dir.iterdir()), 1):
                if not sd.is_dir():
                    continue
                name = sd.name
                rules_count = '-'
        ...
        # Skills table
        L.append('## Layer 1：原生 Skills（.claude/skills/）')
        L.append('')
        L.append('每個 SKILL.md frontmatter 含 `triggers` 觸發詞列表，自動路由匹配。')
        L.append('')
