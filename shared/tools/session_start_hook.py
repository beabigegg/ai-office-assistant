"""
UserPromptSubmit hook: inject session_start reminder if no active workflow exists.
Fires on every user message; only outputs when session_start is needed.
"""
import json
import sys
import os
...
STATE_FILE = os.path.join(WORKSPACE_DIR, "shared", "workflows", ".workflow_state.json")
...
def main():
    ...
    if "projects/" in cwd_posix:
        ...
        if len(parts) > 1:
            ...
            project_hint = f"projects/{project_name}"

    hint = (
        "[SESSION] 尚未啟動 session_start workflow。\n"
        f"建議執行：python shared/workflows/coordinator.py start session_start "
