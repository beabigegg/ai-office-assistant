...
def _get_macro_dir() -> str:
    if platform.system() == "Darwin":
        ...
        return os.path.join(os.environ.get("APPDATA", ""), "LibreOffice", "4", "user", "basic", "Standard")
    else:
...
def recalc(filename, timeout=30):
    ...
    if platform.system() == "Linux":
        result = subprocess.run(
            ["timeout", str(timeout), "soffice"] + soffice_args,
            capture_output=True, text=True, env=get_soffice_env()
        )
    elif platform.system() == "Darwin" and has_gtimeout():
        result = subprocess.run(
            ["gtimeout", str(timeout), "soffice"] + soffice_args,
            capture_output=True, text=True, env=get_soffice_env()
        )
    else:
        result = run_soffice(soffice_args, capture_output=True, text=True, timeout=timeout)
